...
io.on('connection', (socket) => {
	socket.on('join', ({ name, room }, callback) => {
		...
		}); // broadcast function make a event to all users in the room, then emit the message new user joined

		io.to(user.room).emit('roomData', {
			room: user.room,
			users: getUsersInRoom(user.room),
		}); // send to frontend for the current users in the room

		callback();
	});

	socket.on('sendMessage', (message, callback) => {
		// send message event, takes in the message, and a callback function
		const user = getUser(socket.id); // find the current user

		io.to(user.room).emit('message', {
			user: user.name,
			text: message,
		}); // send an io instance to the user's room with the message object, user(name) and test(message)
